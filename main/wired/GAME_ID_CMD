...
static void nsi_sd_info_cmd_hdlr(uint8_t channel, uint8_t port, uint16_t item) {
    struct raw_fb fb_data = {0};
    ...
    fb_data.header.wired_id = channel;
    fb_data.header.type = FB_TYPE_SD_INFO;
    fb_data.header.data_len = 8;
    for (uint32_t i = 0; i < 8; ++i) {
        fb_data.data[i] = buf[i];
    }
...
static void gc_pad_cmd_hdlr(uint8_t channel, uint8_t port, uint16_t item) {
    ...
    switch (buf[0]) {
        case GAME_ID_CMD:
            ...
            break;
        case SD_INFO_CMD:
            nsi_sd_info_cmd_hdlr(channel, port, item);
            break;
        case 0x00:
        case 0xFF:
